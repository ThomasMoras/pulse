...
import { Gender, SBTMetaData } from "./Type";
...
describe("PulseSBT", function () {
    ...
    describe("Mint SBT", function () {
        ...
        it("Should revert, only Pulse contract can mint SBT", async () => {
            ...
            await expect(
                sbtContract.mintSoulBoundToken(user1.address, metadata),
            ).to.be.revertedWith(
        ...
        it("Should mint and emit event for user 1", async () => {
            ...
            await expect(mintTx)
                ...
                .withArgs(user1.address, 1); // Le premier token aura l'ID 1
        });
    ...
    describe("Mint SBTs and check SBT Data", function () {
        ...
        it("Should verify SBT ownership", async () => {
            assert(
                (await sbtContract.hasSoulBoundToken(user1.address)) === true,
            );
            assert(
                (await sbtContract.hasSoulBoundToken(user2.address)) === true,
            );
            assert(
                (await sbtContract.hasSoulBoundToken(user3.address)) === true,
            );
        });
        ...
        it("Should update SBT meta data according to user and emit event", async () => {
            ...
            expect(metadata.interestedBy).to.deep.equal(
                metadataModified1.interestedBy,
            );
            expect(metadata.localisation).to.equal(
                metadataModified1.localisation,
            );
            expect(metadata.hobbies).to.deep.equal(metadataModified1.hobbies);
            expect(metadata.note).to.equal(metadataModified1.note);
            expect(metadata.ipfsHashs).to.deep.equal(
                metadataModified1.ipfsHashs,
            );
        });
    });
